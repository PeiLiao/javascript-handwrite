...
function isPostOrder(arr) {
  ...
  function insert(n: number, node?: TreeNode) {
    if (!root) { root = new TreeNode(n); return }
...
function isPostOrder2(arr: number[]) {
  ...
  var mid = arr[arr.length - 1], flag = 0, k = -1;
  ...
  return isPostOrder2(arr.slice(0, k + 1)) && isPostOrder2(arr.slice(k, arr.length - 1))
}
...
function sumPath(pRoot: TreeNode, sum) {
  ...
  while (parr.length > 0) {
    ...
    const { node: { left, right, value }, r, arr } = cur;
    if (value < r) {
...
function getDepth(pRoot) {
  ...
  while (parr.length > 0) {
    depth++
    parr = parr.map(p => [p.left, p.right]).reduce((a, b) => a.concat(b)).filter(p => !!p)
  }
  return depth
}
