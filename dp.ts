...
function maxArea_recursion(m: number, n: number) {
  ...
  return Math.max(
    ...[...Array(m).keys()].map((k) => k * maxArea_recursion(m - k, n - 1))
  );
}
...
function palindromes_continuous(str: string) {
  ...
  for (let end = 0; end < n; end++) {
    for (let start = 0; start <= end; start++) {
      dp[start][end] =
        str[start] === str[end] && (start >= end - 2 || dp[start + 1][end - 1])
...
function palindromes_discrete(str: string) {
  ...
  for (let end = 0; end < n; end++) {
    for (let start = end; start >= 0; start--) {
      ...
      if (start >= end - 1) {
        dp[start][end] =
          str[start] === str[end] ? str.substring(start, end + 1) : str[start];
        substrs.push(dp[start][end]);
        continue;
      }
      ...
      if (str[start] === str[end]) {
        ...
        dp[start][end] =
          dp[start][end - 1].length > dp[start + 1][end].length
            ? dp[start][end - 1]
            : dp[start + 1][end];
      }
    }
...
function availableSquare_WFS(k: number, rows: number, cols: number) {
  ...
  path.forEach((p) => console.log(p.reduce((a, b) => `${a}${b}`)));
  // console.log(path);
  return t;
}
    ...
    for (let j = 0; j < array.length; j++) {
      if (n === 0) {
        ...
        for (let k = n; k < j; k++) {
          dp[n][j] = Math.max(
            dp[n][j],
            dp[n - 1][k] + maxSumSubArray(array.slice(k + 1, j + 1))
          );
        }
      }
    }
